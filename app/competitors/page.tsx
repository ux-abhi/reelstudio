...
export default function CompetitorsPage() {
  ...
  async function analyzeCompetitor() {
    ...
    try {
      // Try Apify first (real scraping), fall back to Groq-only analysis
      ...
      // Apify not configured — Groq-only fallback
      const prompt = `Analyse this Instagram account as a competitor to a UX designer and HCI master's student creating content about Figma, Framer, AI design tools, and design education.
Handle: @${handle}
Trends: ${JSON.stringify(scan?.trendPulse?.slice(0, 5) ?? [])}
Return ONLY this JSON (no markdown):
{
  "handle": "@${handle}",
  "strategy": "2-3 sentences on their content strategy",
  "hooksToSteal": ["hook 1 adapted to your voice", "hook 2", "hook 3", "hook 4", "hook 5"],
  "gapsTheyLeave": ["gap 1", "gap 2", "gap 3"],
  "threatLevel": "high",
  "threatReason": "specific reason",
  "collabScore": "7/10",
  "collabPitch": "exact DM message ready to copy-paste",
  "whitespace": "the one angle they never touch that you can own"
}`
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                ...
                <button
                  ...
                  className="btn-ghost"
                  style={{ fontSize: 13, width: 'auto', padding: '5px 10px' }}
                  title="Remove"
                >
                  ✕
                </button>
